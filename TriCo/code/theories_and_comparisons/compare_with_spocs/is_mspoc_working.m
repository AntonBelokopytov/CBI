% Проверка, работает ли mSPoC на реальных данных
...
Z1 = squeeze(mean(Z_epochs(:,velocity_chs,:), 1));

% Average across selected channels
Z2 = squeeze(mean(Z_epochs(:,6:8,:).^2, 1));
Z2 = mean(Z2,1);
...
% [We, Ae, Vf, Vz, corrs_espoc, Feat, Epochs_cov, eigenvalues] = espoc(X_epochs, Z2);
% [We, Ae] = env_corrca(X_trials, Fs, 0.25, 0.1);

[We,~,~,Ae] = mspoc(X_epochs,Z2);
[Wsp,Asp] = spoc(X_epochs,Z2);

corr_sp = [];
for i=1:size(Wsp,2)
    for j=1:size(Epochs_cov,3)
        Envs(i,j) = Wsp(:,i)'*Epochs_cov(:,:,j)*Wsp(:,i);
    end
    corr_sp(i,:) = corr(Envs(i,:)',Z2');
end
...
e_idx = 1;
sp_idx = 1;
% w_e = squeeze(We(1,:,e_idx))';
% a_e = squeeze(Ae(1,:,e_idx))';
w_e = squeeze(We(:,e_idx));
a_e = squeeze(Ae(:,e_idx));
