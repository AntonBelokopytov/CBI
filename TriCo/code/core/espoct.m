function [W, A, Vf, Vz, corrs, Feat, Epochs_cov, eigenvalues] = espoct(X_epochs, Z, varargin)
...
[t_s,n_ch,n_ep] = size(X_epochs);
% --- 
Epochs_cov = zeros(n_ch,n_ch,n_ep);
for ep_idx = 1:n_ep
...
% raw матрица для PCA
X_raw = reshape(permute(X_epochs,[2 1 3]), n_ch, []);
[~, Ux] = project_to_pc(X_raw, opt.X_min_var_explained);

n_pc = size(Ux,2);
X_epochsdr = zeros(t_s, n_pc, n_ep);

for ep = 1:n_ep
    X_epochsdr(:,:,ep) = X_epochs(:,:,ep) * Ux;
end

% ---
[Feat,Cxxt] = get_tang_vecs(X_epochsdr,opt);
Cff = cov(Feat');

[Featdr, Uf] = project_to_pc(Feat, 1);
...
for global_src_idx=1:size(Af,2)
    [w, a, s] = project_to_manifold(Af(:,global_src_idx), Cxxt, Cxx, Ux, opt);
...
function [F, Cxxt, Epochs_cov] = get_tang_vecs(X_epochs, opt)
    [~, n_channels, n_epochs] = size(X_epochs);
    Epochs_cov = zeros(n_channels, n_channels, n_epochs);
    
    for ep_idx = 1:n_epochs
        % 1. Считаем сырую эмпирическую ковариацию
        Xcov = cov(X_epochs(:,:,ep_idx));
        
        Xcov_spd = project_to_spd(Xcov, opt.whitening_reg);
        
        Epochs_cov(:,:,ep_idx) = Xcov_spd;
    end
    
    Cxxt = riemann_mean(Epochs_cov);
    F = Tangent_space(Epochs_cov, Cxxt);
    F = F - mean(F,2);
end
...
function [W, A, s] = project_to_manifold(V, Cxxt, Cxx, Ux, opt)
% Project filters to manifold
WW = UnTangent_space(V, Cxxt);
...
% reg_term = opt.whitening_reg * trace(Cxx) / size(Cxx,1);
% Cxx_reg = Cxx + reg_term * eye(size(Cxx,1));

% Normalization and pattern recovery
for local_src_idx=1:size(Uw,2)
    % Return filters from the whightened space
    wi = Ux * Uw(:,local_src_idx);
    % Normalize
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function C_spd = project_to_spd(C, tol)
    % Проекция симметричной матрицы в пространство SPD
    % (Strictly Positive Definite)
    
    if nargin < 2
        tol = 1e-12; % Относительный порог
    end
    
    % 1. Гарантируем строгую симметрию (убираем ошибки float)
    C = (C + C') / 2;
    ...
    % 3. Вычисляем абсолютный порог на основе максимального значения
    abs_tol = max(d) * tol;
    
    % 4. Если есть "плохие" собственные значения - проецируем
    if any(d < abs_tol)
        d(d < abs_tol) = abs_tol; % Подтягиваем всё, что меньше порога
        C_spd = V * diag(d) * V';
        C_spd = (C_spd + C_spd') / 2; % Снова гарантируем симметрию
    else
        C_spd = C; % Если всё и так хорошо, ничего не трогаем
    end
end
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
