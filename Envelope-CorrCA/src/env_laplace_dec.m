function [W, A, eigenvalues, Epochs_cov, z] = env_laplace_dec(X, ...
Fs, Wsize, Ssize)
...
for i=1:n_epochs
    for j=1:n_trials
        Epochs_cov(:,:,i,j) = cov(X_epochs(:,:,i,j));
    end
end
...
N_neigb = n_epochs;
[L, D] = laplace_embedding(mX_covsW, N_neigb);
...
function [L, D, W] = laplace_embedding(Covs, N_neigb)
    n = size(Covs,3);
    Dists = zeros(n);
    for i=1:n-1
        for j=i+1:n
    ...
    for i = 1:n
        distances_i = Dists(i, :);
        [sorted_dists, sorted_idx] = sort(distances_i, 'ascend');
        ...
        k_limit = min(n, N_neigb + 1);
        
        for iter = 1:50 
            k_dists = sorted_dists(2:k_limit);
            vals = exp(-max(0, k_dists - rho_i) / sigma_i);
            sum_vals = sum(vals);
        ...
        knn_idx = sorted_idx(2:k_limit);
        W_directed(i, knn_idx) = exp(-max(0, distances_i(knn_idx) - rho_i) / sigma_i);
    end
    
    W_n = W_directed + W_directed' - (W_directed .* W_directed');
    W = W_n - diag(diag(W_n));
